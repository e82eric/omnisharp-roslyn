...
namespace OmniSharp.Roslyn.CSharp.Services.Decompilation
{
    ...
    public class DecompilationExternalSourceService : BaseExternalSourceService, IExternalSourceService
    {
        ...
        private readonly ILSpayCSharpDecompiledSourceService _decompiler;
        ...
        public DecompilationExternalSourceService(ILoggerFactory loggerFactory) : base()
        {
            ...
            _decompiler = new ILSpayCSharpDecompiledSourceService();
        }

        public async Task<(Document document, string documentPath)> GetAndAddExternalSymbolDocument(Project project, string rootFileTypeName, string assemblyFilePath, string fileName)
        {
        ...
        public async Task<(Document document, string documentPath)> GetAndAddExternalSymbolDocument(Project project, ISymbol symbol, CancellationToken cancellationToken)
        {
            ...
            if (!_cache.TryGetValue(fileName, out var document))
            {
                ...
                var x = topLevelSymbol.ContainingAssembly;
                document = await _decompiler.AddSourceToAsync(temporaryDocument, compilation, topLevelSymbol, string.Empty, cancellationToken);
